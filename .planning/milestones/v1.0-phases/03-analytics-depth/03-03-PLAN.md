...
def get_winrate_by_periodo(conn, liga=None, entrada=None, date_start=None, date_end=None) -> list:
    """Retorna lista de {periodo, greens, total, win_rate}"""
...
<task type="auto">
  <name>Task 1: Layout — tabs, badge, modal + imports + helper functions</name>
  <files>helpertips/dashboard.py</files>
  <read_first>
    - helpertips/dashboard.py (layout completo atual — entender estrutura e posicao de insercao)
...
**5. Helper functions para figuras (adicionar ANTES dos callbacks):**
...
**Resumo das modificacoes nesta task:**
1. Imports: adicionar 9 novas funcoes de queries.py + get_stats de store.py + State, no_update, ctx de dash
...
5. 7 helper functions (_make_heatmap_figure, _make_equity_figure, _make_dow_figure, _make_gale_figure, _make_volume_figure, _format_streak, _build_cross_table)
  </action>
  <verify>
    <automated>cd /Users/luciano/helpertips && python3 -c "from helpertips.dashboard import app; print('Import OK')"</automated>
  </verify>
  <done>Layout do dashboard atualizado com tabs, badge, modal e 7 helper functions. Import do modulo funciona sem erro.</done>
</task>

<task type="auto">
  <name>Task 2: Callbacks — update_tabs, toggle_modal, badge no master + testes</name>
  <files>helpertips/dashboard.py, tests/test_dashboard.py</files>
  <read_first>
    - helpertips/dashboard.py (estado apos Task 1 — verificar posicao dos callbacks existentes)
    - tests/test_dashboard.py (estado atual — entender padrao de testes e IDs verificados)
    - .planning/phases/03-analytics-depth/03-VALIDATION.md (IDs requeridos)
  </read_first>
  <action>
**Parte A — Callbacks em dashboard.py:**

**1. Callback separado para abas (D-03 — lazy render via active_tab):**
...
**3. Badge no master callback (D-19, D-20, D-22):**
...
**Parte B — Testes em tests/test_dashboard.py:**

**4. Atualizar `test_layout_has_required_component_ids`** em `tests/test_dashboard.py` para incluir os novos IDs de componentes criados nesta fase. Adicionar a verificacao dos seguintes IDs: `tabs-analytics`, `badge-coverage`, `modal-parse-failures`, `graph-heatmap`, `graph-equity`, `graph-dow`, `graph-gale`, `graph-volume`, `kpi-streak-current`, `kpi-streak-max-green`, `kpi-streak-max-red`, `table-cross-dimensional`.

**5. Adicionar test_coverage_badge_thresholds** — teste unitario que valida a logica de threshold do badge:
```python
...
**6. Adicionar test_format_streak** — testar helper _format_streak:
```python
...
```
  ...
  <done>Callbacks integrados (update_tabs, toggle_modal, badge no master). Testes atualizados: test_layout_has_required_component_ids inclui os 12 novos IDs (tabs-analytics, badge-coverage, modal-parse-failures, graph-heatmap, graph-equity, graph-dow, graph-gale, graph-volume, kpi-streak-current, kpi-streak-max-green, kpi-streak-max-red, table-cross-dimensional), test_coverage_badge_thresholds e test_format_streak adicionados. Suite completa verde.</done>
</task>
